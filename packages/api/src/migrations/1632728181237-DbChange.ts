...
export class DbChange1632728181237 implements MigrationInterface {
    name = 'DbChange1632728181237'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`evergarden\`.\`stories\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created\` timestamp NOT NULL, \`updated\` timestamp NOT NULL, \`slug\` varchar(255) NOT NULL, \`title\` varchar(255) NOT NULL, \`description\` varchar(8000) NULL, \`thumbnail\` varchar(2000) NULL, \`cover\` varchar(2000) NULL, \`status\` enum ('full', 'ongoing') NOT NULL DEFAULT 'ongoing', \`type\` enum ('convert', 'translate', 'self-composed') NOT NULL DEFAULT 'translate', \`view\` int NOT NULL DEFAULT '0', \`upvote\` int NOT NULL DEFAULT '0', \`downvote\` int NOT NULL DEFAULT '0', \`lastChapter\` int NULL, \`published\` tinyint NOT NULL DEFAULT 0, \`createdById\` int NULL, \`updatedById\` int NULL, UNIQUE INDEX \`IDX_0e45709567c3d81765b64ba115\` (\`slug\`), UNIQUE INDEX \`IDX_18e913d264452630f78e9209e8\` (\`title\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_0e45709567c3d81765b64ba115\` ON \`evergarden\`.\`stories\``);
