...
export class DbChange1628771807089 implements MigrationInterface {
    name = 'DbChange1628771807089'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`evergarden\`.\`histories\` (\`id\` int NOT NULL AUTO_INCREMENT, \`currentChapterNo\` int NOT NULL DEFAULT '0', \`started\` datetime NOT NULL, \`lastVisit\` datetime NOT NULL, \`currentReadingPosition\` float NOT NULL DEFAULT '0', \`vote\` enum ('upvote', 'downvote', 'none') NULL DEFAULT 'none', \`isFollowing\` tinyint NOT NULL DEFAULT 0, \`storyId\` int NOT NULL, \`userId\` int NOT NULL, UNIQUE INDEX \`IDX_1403ddb5a860084e25b727991f\` (\`storyId\`, \`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`evergarden\`.\`chapters\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created\` datetime NOT NULL, \`updated\` datetime NOT NULL, \`chapterNo\` int NOT NULL, \`title\` varchar(255) NULL, \`content\` mediumtext NOT NULL, \`published\` tinyint NOT NULL DEFAULT 0, \`storyId\` int NOT NULL, \`createdById\` int NULL, \`updatedById\` int NULL, UNIQUE INDEX \`IDX_7f34072feacf768b64cbc505a0\` (\`storyId\`, \`chapterNo\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`evergarden\`.\`stories\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created\` datetime NOT NULL, \`updated\` datetime NOT NULL, \`url\` varchar(255) NOT NULL, \`title\` varchar(255) NOT NULL, \`description\` varchar(2000) NULL, \`thumbnail\` varchar(2000) NULL, \`cover\` varchar(2000) NULL, \`status\` enum ('full', 'ongoing') NOT NULL DEFAULT 'ongoing', \`view\` int NOT NULL DEFAULT '0', \`upvote\` int NOT NULL DEFAULT '0', \`downvote\` int NOT NULL DEFAULT '0', \`lastChapter\` int NULL, \`published\` tinyint NOT NULL DEFAULT 0, \`createdById\` int NULL, \`updatedById\` int NULL, UNIQUE INDEX \`IDX_6f534ae2896f60a912f74b5599\` (\`url\`), UNIQUE INDEX \`IDX_18e913d264452630f78e9209e8\` (\`title\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
